...
export const DepositeModal = ({
	...
	const iidd = new Date().getTime()
	const transactionState = useMyTransactionState(String(iidd));
	...
	useEffect(() => {
		if (transactionState.type === "waitingForConfirmation" && transactionState.id === String(iidd) && transactionState.tx?.rawSentTransaction) {
			onDone(transactionState.tx.rawSentTransaction as unknown as string);
		}
	}, [transactionState.type])
	console.debug("transactionState =", transactionState.id, transactionState.type, transactionState);
