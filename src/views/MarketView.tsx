...
export const MarketView = ({
	...
	const { publicClient, walletClient, chainId, liquity, account } = useLiquity();
	...
	useEffect(() => {
		const func = async () => {
			if (trove.status === "closedByLiquidation" || trove.status === "closedByRedemption") {
				const res: bigint = await publicClient?.readContract({
					address: liquity.connection.addresses.collSurplusPool as Address,
		...
		func();
	}, [account, liquity.connection.addresses.collSurplusPool, publicClient, trove]);
	...
	return <>
		<div
			...
			<div>
				{(
					...
					(trove.status === "closedByRedemption" && collateralFromCollSurplusPool.eq(0))
						...
						<button
							...
							disabled={accountBalance.eq(0) || trove.status === "closedByLiquidation" || trove.status === "closedByRedemption"}>
				...
				{(
					trove.status === "open" ||
					((trove.status === "closedByLiquidation" || trove.status === "closedByRedemption") && collateralFromCollSurplusPool.gt(0))
				) && <div
					className="card"
						...
						<div
							...
							<div
								...
								<button
									...
									onClick={handleDeposit}
									disabled={trove.status === "closedByLiquidation" || trove.status === "closedByRedemption"}>
						...
						<div
							...
							<div
								...
								<button
									...
									disabled={trove.status === "open" && availableWithdrawal.lt(0.01)}>
		...
		{showWithdrawModal && <WithdrawModal
			...
			liquidationPoint={appLiquidationPoint}
			availableBorrow={availableBorrow}
			collateralFromCollSurplusPool={collateralFromCollSurplusPool} />}
